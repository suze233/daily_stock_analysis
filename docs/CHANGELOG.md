...
- [改进] 全球市场日报改用 GitHub Models 在每次工作流中读取当日行情、新闻与媒体公开摘要，动态生成报道要点、市场影响和后续观察；生成失败时明确降级，不再使用固定分析模板。
- [改进] 全球市场日报以 VIX（CBOE 波动率指数）替换纳斯达克综合指数，并为 VIX 提供独立的避险与波动率解读。
- [改进] “报道要点”统一改为简短中文事件总结，并以“市场影响”说明传导路径；不再显示或回退到报道标题。
- [修复] 过滤 Google News 跳转页的通用重复描述；当原文摘要不可读取时，根据每条报道的具体事件生成独立中文新闻要点。
- [改进] “今日解读”新增报道要点、影响机制与后续观察三段式说明，分别呈现事实线索、对价格的传导路径及下一步验证指标。
- [改进] “今日解读”抓取原报道可公开读取的摘要/导语作为“报道要点”；摘要不可用时明确提示阅读原文，避免仅以标题充当内容说明。
