...
“今日解读”不会使用固定原因模板。GitHub Actions 每次更新时会解析当日新闻原文链接和媒体公开摘要，并通过仓库 Secret `OPENAI_API_KEY`（可配合 `OPENAI_BASE_URL`、`OPENAI_MODEL`）或 `DEEPSEEK_API_KEY` 动态生成中文报道要点、市场影响、后续观察，以及“今日市场脉搏”中的后续可关注板块。未配置密钥或生成失败时，页面会明确显示“实时解读暂不可用”，且不会展示虚构的关注板块。
