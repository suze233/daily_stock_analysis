...
export default function PublicMarketPage() {
  ...
  return <main className="public-market">
    ...
    <section className="public-section" id="analysis"><div className="public-section-head"><div><span>02</span><h2>今日解读</h2></div><p>行情方向 × 可核对的公开报道</p></div><div className="public-analysis"><article className="public-lead"><span>核心叙事</span><h3>{equity.map((item) => item.reason).join('；')}</h3><p>{equity.map((item) => item.detail).join(' ')}</p><div className="public-evidence-list">{equity.map((item) => <EvidenceLink key={item.symbol} market={item} />)}</div><div>{Array.from(new Set(equity.flatMap((item) => item.tags))).slice(0, 4).map((tag) => <b key={tag}>{tag}</b>)}</div></article><div className="public-reasons">{commodities.map((item) => <article key={item.symbol}><span>{item.name}</span><h3>{item.reason}</h3><p>{item.detail}</p><EvidenceLink market={item} /><div>{item.tags.map((tag) => <b key={tag}>{tag}</b>)}</div></article>)}</div></div><p className="public-caveat">原因分析表示与当日行情方向一致的可能驱动因素，不代表已经证实的单一因果关系，也不构成投资建议。</p></section>
